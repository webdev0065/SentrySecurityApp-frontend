...
const AgencyDetailsScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const handleContinue = async () => {
    ...
    if (gstNumber && !/^[A-Z0-9]{15}$/.test(gstNumber.trim().toUpperCase())) {
      Alert.alert('Invalid GST number', 'Enter a valid 15-character GST number or leave it blank.');
      return;
    }
    ...
    try {
      ...
      registrationDraft.setProfile({
        agencyName: agencyName.trim(), businessType, gstNumber: gstNumber.trim().toUpperCase(),
        officeAddress: officeAddress.trim(), city: city.trim(), state, pincode,
      });
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        >
          <Text style={[styles.selectText, !businessType && styles.placeholder]}>
        ...
        {businessOpen && (
          <View style={styles.businessMenu}>
            {BUSINESS_TYPES.map(item => (
              ...
              >
                <Feather name={item.icon} size={scaleFont(21)} color={colors.gold} />
        ...
        >
          <Text style={styles.continueText}>{isSubmitting ? 'Saving...' : 'Continue'}</Text>
        </TouchableOpacity>
      </ScrollView>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  heading: {
    position: 'absolute', left: scaleWidth(23), top: scaleHeight(190), width: scaleWidth(355),
    textAlign: 'center', fontSize: scaleFont(42), lineHeight: scaleFont(51), fontWeight: '800', color: colors.primary,
  },
  subtitle: {
    position: 'absolute', left: scaleWidth(23), top: scaleHeight(239), width: scaleWidth(355),
    textAlign: 'center', fontSize: scaleFont(20), lineHeight: scaleFont(24), color: colors.primary,
  },
  input: {
    position: 'absolute', left: scaleWidth(19), width: scaleWidth(363), height: scaleHeight(50),
    borderWidth: 1, borderColor: '#000000', borderRadius: 8, paddingHorizontal: scaleWidth(13),
    fontSize: scaleFont(16), color: colors.primary,
  },
  labelAgencyName: { position: 'absolute', left: scaleWidth(23), top: scaleHeight(292), fontSize: scaleFont(22), fontWeight: '500', color: colors.primary },
  agencyNameInput: { top: scaleHeight(322) },
  labelBusinessType: { position: 'absolute', left: scaleWidth(23), top: scaleHeight(396), fontSize: scaleFont(22), fontWeight: '500', color: colors.primary },
  businessInput: { top: scaleHeight(426), flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  selectText: { fontSize: scaleFont(16), color: colors.primary },
  placeholder: { color: '#A3A3A3' },
  businessMenu: {
    position: 'absolute', left: scaleWidth(19), top: scaleHeight(484), width: scaleWidth(363), height: scaleHeight(205),
    backgroundColor: colors.white, borderWidth: 1, borderColor: colors.primary, borderRadius: 8, overflow: 'hidden',
    paddingVertical: scaleHeight(7), zIndex: 20, elevation: 12, shadowColor: '#000', shadowOpacity: 0.22, shadowRadius: 12, shadowOffset: { width: 3, height: 9 },
  },
  businessOption: { height: scaleHeight(32), flexDirection: 'row', alignItems: 'center', paddingHorizontal: scaleWidth(9), gap: scaleWidth(9) },
  businessOptionText: { fontSize: scaleFont(20), color: '#000000' },
  labelGst: { position: 'absolute', left: scaleWidth(23), top: scaleHeight(497), fontSize: scaleFont(22), fontWeight: '500', color: colors.primary },
  gstInput: { top: scaleHeight(524) },
  labelOfficeAddress: { position: 'absolute', left: scaleWidth(23), top: scaleHeight(594), fontSize: scaleFont(22), fontWeight: '500', color: colors.primary },
  officeInput: { top: scaleHeight(624) },
  labelCity: { position: 'absolute', left: scaleWidth(27), top: scaleHeight(693), fontSize: scaleFont(22), fontWeight: '500', color: colors.primary },
  cityInput: { left: scaleWidth(19), top: scaleHeight(724), width: scaleWidth(173) },
  labelState: { position: 'absolute', left: scaleWidth(215), top: scaleHeight(693), fontSize: scaleFont(22), fontWeight: '500', color: colors.primary },
  stateInput: { left: scaleWidth(209), top: scaleHeight(724), width: scaleWidth(173), flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  stateMenu: {
    position: 'absolute', left: scaleWidth(19), top: scaleHeight(783), width: scaleWidth(363), height: scaleHeight(238),
    backgroundColor: colors.white, borderWidth: 1, borderColor: colors.primary, borderRadius: 8,
    zIndex: 30, elevation: 12, shadowColor: '#000', shadowOpacity: 0.22, shadowRadius: 12, shadowOffset: { width: 3, height: 9 },
  },
  stateSearch: { height: scaleHeight(39), borderBottomWidth: 1, borderBottomColor: colors.primary, paddingHorizontal: scaleWidth(13), flexDirection: 'row', alignItems: 'center', gap: scaleWidth(7) },
  searchInput: { flex: 1, height: '100%', padding: 0, fontSize: scaleFont(20), color: colors.primary },
  stateList: { flex: 1 },
  stateOption: { height: scaleHeight(31), justifyContent: 'center', paddingHorizontal: scaleWidth(23) },
  stateOptionText: { fontSize: scaleFont(20), color: '#000000' },
  labelPincode: { position: 'absolute', left: scaleWidth(23), top: scaleHeight(796), fontSize: scaleFont(22), fontWeight: '500', color: colors.primary },
  pincodeInput: { top: scaleHeight(826) },
  continueButton: { position: 'absolute', left: scaleWidth(19), top: scaleHeight(950), width: scaleWidth(363), height: scaleHeight(50), borderRadius: 8, backgroundColor: colors.primary, alignItems: 'center', justifyContent: 'center' },
